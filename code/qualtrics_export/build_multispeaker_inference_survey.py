#!/usr/bin/env python3
"""Build a multi-speaker inference survey for Qualtrics.

Current available default selection:
  - speaker0: f5 items 0-9
  - speaker1: ns2 items 0-7
  - speaker2: ns3 items 0-7
"""
...
from qualtrics_inference_export import build_survey, print_preview


DEFAULT_SELECTIONS = ("speaker0:f5:0-9", "speaker1:ns2:0-7", "speaker2:ns3:0-7")
...
def parse_selection(raw_selection: str) -> tuple[str, str, list[int]]:
    """Parse selection specs like speaker1:ns2:0-7."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a multi-speaker inference survey for Qualtrics."
    )
    parser.add_argument(
        "--selection",
        action="append",
        help=(
            "Selection in speaker:source:item_range form, e.g. speaker1:ns2:0-7. "
            "May be repeated. Defaults to the current available inference survey."
        ),
    )
    parser.add_argument(
        "--stimuli-dir",
    ...
    parser.add_argument(
        "--audio-dir",
        type=Path,
        default=Path("data/clips/inference_multispeaker"),
        help="Directory where selected audio clips are copied for Qualtrics upload",
    )
    parser.add_argument(
        "--input-json",
        type=Path,
        default=Path("code/qualtrics_export/input/inference_multispeaker/items.json"),
        help="Path to write the generated JSON subset",
    )
    parser.add_argument(
        "--output-txt",
        type=Path,
        default=Path(
            "code/qualtrics_export/output/inference_multispeaker/"
            "inference_multispeaker_survey.txt"
        ),
...
def main() -> None:
    ...
    items = build_inference_items(selections, args.stimuli_dir, args.speakers_dir, args.audio_dir)

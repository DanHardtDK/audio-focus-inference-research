#!/usr/bin/env python3
"""Build a survey JSON subset from clip filenames like f1_item0.wav or ns1_item0.wav."""
...
CLIP_PATTERN = re.compile(r"^(?:(speaker\d+)_)?((?:f|ns)\d+)_item(\d+)\.wav$", re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "clips_dir",
        type=Path,
        help="Directory containing clip files like f1_item0.wav or speaker0_f1_item0.wav",
    )
    ...
    parser.add_argument(
        ...
        help="Directory containing the source fX.json and nsX.json files",
    )
...
def build_subset(clips_dir: Path, input_dir: Path) -> list[dict[str, Any]]:
    ...
    for clip_path in clip_paths:
        ...
        source_stem = match.group(2).lower()
        source_item_index = int(match.group(3))
